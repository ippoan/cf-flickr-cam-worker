...
**`npx wrangler deploy` は D1 migration を自動適用しない。** CI の
`deploy_staging_script` / `deploy_release_script` はどちらも `wrangler deploy`
のみなので、`migrations/` 配下の SQL はデプロイのたびに自動で remote D1
(`cf-flickr-cam-worker-db`) へ反映されるわけではない (Refs #12 — cron 側
`D1_ERROR: no such table: cam_files` の原因)。

CI の `CLOUDFLARE_API_TOKEN` は (`ref-files-worker` と同様) `Workers
Scripts:Edit` scope のみを想定しており `D1:Edit` を持たない可能性が高いため、
migration の remote 適用は **CI に組み込まず、developer workstation から
手動実行**する:

```bash
npm run d1:migrate:prod   # wrangler d1 migrations apply cf-flickr-cam-worker-db --remote
```

新しい migration ファイルを追加した時は、コミット・マージ後に上記コマンドを
手動で 1 回実行すること。
