...
4. secrets 投入 (`wrangler secret put` は使わない — secrets-inventory の drift
   ...
   - `FLICKR_CONSUMER_KEY` / `FLICKR_CONSUMER_SECRET` / `CAM_DIGEST_USER` /
     `CAM_DIGEST_PASS` / `CAM_MACHINE_NAME` の 5 つは、同じカメラ・同じ Flickr
     App を使っていた `ippoan/rust-flickr` (2026-07-08 廃止済み) が GCP に持って
     いた `rust-flickr-*` secret をそのまま再利用する (値の再入力不要、投入済み)
   - `CAM_HOST` (カメラの内部 IP) / `OAUTH_STATE_SECRET` は本 repo 固有の新規
     secret。値を `secret-inject` skill (stdin 経由) で投入済み。カメラの CGI
     パス (`snapshot.cgi` 等、秘匿性なし) は secret ではなく `wrangler.jsonc`
     の `vars` (`CAM_SDCARD_CGI_PATH` 等) に平文で置く (Refs #17)
