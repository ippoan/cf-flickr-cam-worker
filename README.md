...
4. `FLICKR_CONSUMER_KEY` / `FLICKR_CONSUMER_SECRET` / `CAM_DIGEST_USER` /
   `CAM_DIGEST_PASS` / `CAM_MACHINE_NAME` / `CAM_SDCARD_CGI` / `CAM_MP4_CGI` /
   `CAM_JPG_CGI` / `OAUTH_STATE_SECRET` (openssl rand -hex 32 等でランダム生成)
   を `secret-inject` skill で CF Secrets Store に投入 (`wrangler secret put` は
   使わない — secrets-inventory の drift 検知対象に含めるため)、`wrangler.jsonc`
   の `secrets_store_secrets` `store_id` を反映
